...
def _add_items_to_board(
    ...
    try:
        for _, row in df.iterrows():
            ...
            while retry_count < max_retries:
                try:
                    column_values: Dict[str, Union[str, Dict[str, Any]]] = {}
                    for col in df.columns:
                        if col == "name":  # Skip name column as it's handled separately
                            continue
                    ...
                    variables = {
                        "boardId": str(board_id),  # Changed from board_id to boardId
...
def _fetch_board_metadata(
    ...
    board_data: Dict[str, Any] = meta_response["data"]["boards"][0]
    return board_data
